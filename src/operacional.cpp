...
void executaOperacao(String cmd) {
    ...
    // Verificar autenticação BLE antes de processar qualquer comando
    #ifdef USAR_ESP32_UART_BLE
        if (!isDeviceAuthenticated()) {
            DBG_PRINTLN(F("\n[OPER] Comando rejeitado - dispositivo não autenticado"));
...
void IRAM_ATTR fluxoISR() {
    contadorPulso++;
    horaPulso = esp_timer_get_time(); // [FIX #2] horaPulso atualizado a cada pulso — usado como referencia do timeout renovavel
